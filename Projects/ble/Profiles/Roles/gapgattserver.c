...
static bStatus_t ggs_ReadAttrCB( uint16 connHandle, gattAttribute_t *pAttr,
...
bStatus_t GGS_AddService( uint32 services )
{
  ...
  if ( services & GAP_SERVICE )
  {
    ...
    status = GATTServApp_RegisterService( gapAttrTbl, GATT_NUM_ATTRS( gapAttrTbl ),
                                          GATT_MAX_ENCRYPT_KEY_SIZE, &gapServiceCBs );
  }
 ...
 *
 * @param       wPermit - write access permissions
...
static void ggs_SetAttrWPermit( uint8 wPermit, uint8 *pPermissions, uint8 *pCharProps )
{
  ...
  if ( gattPermitEncryptWrite( wPermit ) )
  {
    *pPermissions |= GATT_PERMIT_ENCRYPT_WRITE;
  }
  else
  {
    *pPermissions &= ~GATT_PERMIT_ENCRYPT_WRITE;
  ...
  if ( gattPermitWrite( wPermit )        ||
       gattPermitEncryptWrite( wPermit ) ||
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      case DEVICE_NAME_UUID:
        {
          uint16 len = osal_strlen( (char *)(pAttr->pValue) );
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      case DEVICE_NAME_UUID:
        ...
        {
          uint16 curLen = osal_strlen( (char *)(pAttr->pValue) );
        ...
        if ( status == SUCCESS )
        {
          ...
          // Insert NULL terminator and clear remaining bytes
          VOID osal_memset( &(pAttr->pValue[offset]), 0, 
                            ((GAP_DEVICE_NAME_LEN+1) - offset) );

          // Notify application
          if ( ggs_AppCBs && ggs_AppCBs->pfnAttrValueChange )
          {
            ggs_AppCBs->pfnAttrValueChange( connHandle, GGS_DEVICE_NAME_ATT );
          }
        }
        break;
      ...
      case APPEARANCE_UUID:
        ...
        if ( status == SUCCESS )
        {
          ...
          *pCurValue = BUILD_UINT16( pValue[0], pValue[1] );
          ...
          if ( ggs_AppCBs && ggs_AppCBs->pfnAttrValueChange )
          {
            ggs_AppCBs->pfnAttrValueChange( connHandle, GGS_APPEARANCE_ATT );
          }
        }
        break;
      ...
      case RECONNECT_ADDR_UUID:
        ...
        if ( status == SUCCESS )
        {
          ...
          if ( ggs_AppCBs && ggs_AppCBs->pfnAttrValueChange )
          {
            ggs_AppCBs->pfnAttrValueChange( connHandle, GGS_RECONNCT_ADDR_ATT );
          }
        }
        break;
