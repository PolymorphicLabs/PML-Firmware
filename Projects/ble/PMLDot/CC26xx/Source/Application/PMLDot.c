/*******************************************************************************
  ...
  Description:    This file is the PMLDot application's main body.
...
// Sensor devices
#include "st_util.h"
#include "PMLDot_Tmp.h"
#include "PMLDot_AHRS.h"
#include "PMLDot_Bar.h"
#include "PMLDot_Mov.h"
#include "PMLDot_Opt.h"
#include "PMLDot_Keys.h"
#include "PMLDot_IO.h"
...
#ifdef FEATURE_LCD
#include "PMLDot_Display.h"
#endif

// Optional services
#ifdef FEATURE_OAD
  #include "PMLDot_connctrl.h"
...
// GAP GATT Attributes
static const uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "PMLDot 2.0";
...
static void PMLDot_init( void);
static void PMLDot_taskFxn( UArg a0, UArg a1);
static void PMLDot_processStackMsg( ICall_Hdr *pMsg);
static void PMLDot_processGATTMsg(gattMsgEvent_t *pMsg);
static void PMLDot_processAppMsg( stEvt_t *pMsg);
static void PMLDot_processStateChangeEvt( gaprole_States_t newState ) ;
static void PMLDot_processCharValueChangeEvt( uint8_t serviceID, uint8_t paramID ) ;
static void PMLDot_performPeriodicTask( void);
static void PMLDot_stateChangeCB( gaprole_States_t newState);
static void PMLDot_resetAllSensors(void);
static void PMLDot_clockHandler(UArg arg);
static void PMLDot_enqueueMsg(uint8_t event, uint8_t serviceID, uint8_t paramID);
static void PMLDot_callback(PIN_Handle handle, PIN_Id pinId);
static bool PMLDot_hasFactoryImage(void);
static void PMLDot_setDeviceInfo(void);

#ifdef FACTORY_IMAGE
static bool PMLDot_saveFactoryImage(void);
#endif

#ifdef FEATURE_OAD
static void PMLDot_processOadWriteCB(uint8_t event, uint16_t connHandle,
                                           uint8_t *pData);
#endif //FEATURE_OAD
...
// GAP Role Callbacks
static gapRolesCBs_t PMLDot_gapRoleCBs =
{
  PMLDot_stateChangeCB     // Profile State Change Callbacks
};

#ifdef FEATURE_OAD
static gapRolesParamUpdateCB_t paramUpdateCB =
{
  PMLDotConnControl_paramUpdateCB
};

static oadTargetCBs_t simpleBLEPeripheral_oadCBs =
{
  NULL,                       // Read Callback.  Optional.
  PMLDot_processOadWriteCB // Write Callback.  Mandatory.
};
#endif //FEATURE_OAD
...
void PMLDot_createTask(void)
{
  ...
  Task_construct(&sensorTagTask, PMLDot_taskFxn, &taskParams, NULL);
}
...
static void PMLDot_init(void)
{
  ...
  PIN_registerIntCb(hGpioPin, PMLDot_callback);
  ...
  Util_constructClock(&periodicClock, PMLDot_clockHandler,
  ...
  {
    PMLDot_blinkLed(Board_LED_G,TEST_INDICATION_BLINKS);
  }
  else
  {
    PMLDot_blinkLed(Board_LED_G,TEST_INDICATION_BLINKS);
  ...
  if (!PMLDot_hasFactoryImage())
  {
    PMLDot_saveFactoryImage();
  }
#endif

  // Initialize sensors who don't have their own tasks
  PMLDotMov_init();                            // Movement processor
//  PMLDotOpt_init();                            // Light meter

  // Auxiliary services
  PMLDotKeys_init();                           // Simple Keys
  PMLDotIO_init();                             // IO (LED+buzzer+self test)
...
#ifdef FEATURE_LCD
  PMLDotDisplay_init();                        // Display service DevPack LCD
#endif

#ifdef FEATURE_OAD
  PMLDotConnectionControl_init();              // Connection control to
  ...
  // Start the Device
  GAPRole_StartDevice(&PMLDot_gapRoleCBs);
 ...
 * @fn      PMLDot_taskFxn
 *
 * @brief   Application task entry point for the PMLDot
 *
    ...
    if (errno == ICALL_ERRNO_SUCCESS)
    {
                                ...
                                (void **)&pMsg) == ICALL_ERRNO_SUCCESS)
      {
        if ((src == ICALL_SERVICE_CLASS_BLE) && (dest == selfEntity))
        {
          ...
          PMLDot_processStackMsg((ICall_Hdr *)pMsg);
        }
      ...
      PMLDotKeys_processEvent();
//      PMLDotOpt_processSensorEvent();
      PMLDotMov_processSensorEvent();
    }
    ...
    if (!!(events & ST_PERIODIC_EVT))
    {
      ...
      if (gapProfileState == GAPROLE_CONNECTED)
      {
        PMLDot_performPeriodicTask();
      }

      // Blink green LED when advertising
      if (gapProfileState == GAPROLE_ADVERTISING)
      {
        PMLDot_blinkLed(Board_LED_B,1);
        #ifdef FEATURE_LCD
        PMLDot_displayBatteryVoltage();
        #endif
      }
    }
...
static void PMLDot_processAppMsg(stEvt_t *pMsg)
{
  switch (pMsg->event)
  {
    case ST_STATE_CHANGE_EVT:
      PMLDot_processStateChangeEvt((gaprole_States_t)pMsg->serviceID);
      break;

    case ST_CHAR_CHANGE_EVT:
      PMLDot_processCharValueChangeEvt(pMsg->serviceID, pMsg->paramID);
      break;
...
static void PMLDot_stateChangeCB(gaprole_States_t newState)
{
  PMLDot_enqueueMsg(ST_STATE_CHANGE_EVT, newState, NULL);
}

/*******************************************************************************
 * @fn      PMLDot_processStateChangeEvt
 *
...
#ifdef FEATURE_OAD
      PMLDotConnectionControl_update();
#endif
 ...
 * @return  none
 */
void PMLDot_charValueChangeCB(uint8_t serviceID, uint8_t paramID)
{
  PMLDot_enqueueMsg(ST_CHAR_CHANGE_EVT, serviceID, paramID);
}
 ...
 * @return  none
 */
void PMLDot_blinkLed(uint8_t led, uint8_t nBlinks)
{
  uint8_t i;
...
/*******************************************************************************
 * @fn      PMLDot_setLedColor
 *
 * @brief   Turns the LED to the color specified
 * @param   red - red intensity
 * @param   green - green intensity
 * @param   bluw - blue intensity
 *
 * @return  none
 */
void PMLDot_setLedColor(uint8_t red, uint8_t green, uint8_t blue)
{
  uint8_t i;
...
/*******************************************************************************
 * @fn      PMLDot_applyFactoryImage
 *
 * @brief   Load the factory image from external flash and reboot
 *
 * @return  none
 */
void PMLDot_applyFactoryImage(void)
{
  if (PMLDot_hasFactoryImage())
  {
...
/*******************************************************************************
 * @fn      PMLDot_processCharValueChangeEvt
 *
 ...
 * @return  none
 */
static void PMLDot_processCharValueChangeEvt(uint8_t serviceID,
                                                uint8_t paramID)
{
  switch (serviceID)
  {
...
#ifdef FEATURE_OAD
  case SERVICE_ID_CC:
    PMLDotConnControl_processCharChangeEvt(paramID);
    break;
#endif
...
static void PMLDot_processStackMsg(ICall_Hdr *pMsg)
{
  switch (pMsg->event)
  {
    case GATT_MSG_EVENT:
      ...
      PMLDot_processGATTMsg((gattMsgEvent_t *)pMsg);
      break;
...
//  case Board_KEY_LEFT:
//    PMLDotKeys_processKeyLeft();
//    break;

  case Board_KEY_RIGHT:
    PMLDotKeys_processKeyRight();
    break;

//  case Board_RELAY:
//    PMLDotKeys_processRelay();
//    break;
...
/*******************************************************************************
 * @fn      PMLDot_hasFactoryImage
 *
 * @brief   Determine if the PMLDot has a pre-programmed factory image
