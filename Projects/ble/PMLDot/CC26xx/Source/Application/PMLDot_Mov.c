...
static void initCharacteristicValue( uint8_t paramID, uint8_t value,
                                    uint8_t paramLen);
static void PMLDotMov_clockHandler(UArg arg);
 ...
 * @fn      PMLDotMov_init
 *
 * @brief   Initialization function for the PMLDot movement sub-application
 *
 */
void PMLDotMov_init( void )
{
  // Add service
  Movement_addService();
  ...
  {
    PMLDotMov_reset();
    sensorMpu9250RegisterCallback(PMLDotMov_processInterrupt);
  }
  ...
  Util_constructClock(&periodicClock, PMLDotMov_clockHandler,
                      1000, sensorPeriod, false, 0);
}


/*********************************************************************
 * @fn      PMLDotMov_processInterrupt
 *
 * @brief   Interrupt handler for MPU
 *
 */
void PMLDotMov_processInterrupt(void)
{
...
/*********************************************************************
 * @fn      PMLDotMov_processSensorEvent
 *
 * @brief   PMLDot Gyroscope sensor event processor.
 *
 */
void PMLDotMov_processSensorEvent(void)
{
...
/*********************************************************************
 * @fn      PMLDotMov_processCharChangeEvt
 *
 * @brief   PMLDot Movement event handling
 *
 */
void PMLDotMov_processCharChangeEvt(uint8_t paramID)
{
  uint16_t newCfg;
  uint8_t newValue8;
...
{
  // Wake up the application thread
  PMLDot_charValueChangeCB(SERVICE_ID_MOV, paramID);
}
