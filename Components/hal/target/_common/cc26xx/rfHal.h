...
#define RADIO_SETUP_MODE_BLE           0x0000
#define RADIO_SETUP_MODE_15_4          0x0001
...
#if defined( CC26XX_PG1 )
#define SET_RX_TEST_CONFIG( x, v1, v2 )                                        \
  (x) = ((v1)&0x01) | (((v2)&0x01) << 1)
#elif defined( CC26XX ) || defined( CC13XX )
...
#if defined( CC26XX_PG1 )
  uint16         mode;                 // W:  radio mode (BLE, 15.4, etc.) & FS
#elif defined( CC26XX ) || defined( CC13XX )
...
#if defined( CC26XX_PG1 )
  uint8      coarsePrecal;             // W:  coarse calibration value
#endif // CC26XX_PG1
...
PACKED_TYPEDEF_STRUCT
{
  ...
  uint8       endTrig;                 // W:  end trig for Rx
...
PACKED_TYPEDEF_STRUCT
{
  ...
  uint8       endTrig;                 // W:  end trig for Tx
  uint32      syncWord;                // W:  synchronization word
  uint32      endTime;                 // W:  time to end operation
} rfOpCmd_TxTest_t;
